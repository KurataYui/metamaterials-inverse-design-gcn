...
def plot_metrics(metrics_dict, save_path="metrics_summary.png"):
    ...
    plt.plot(epochs, metrics_dict['mape'], label='MAPE (%)')
    plt.xlabel('Epoch')
...
def train_model(num_epochs=100, batch_size=32):
    ...
    with open(csv_path, mode='w', newline='') as file:
        ...
        writer.writerow(["epoch", "train_loss", "val_r2", "val_abs_mae", "val_rel_mae", "val_corr_penalty", "val_mape"])
    ...
    best_r2 = -float('inf')
    metrics = {'r2': [], 'abs_mae': [], 'rel_mae': [], 'corr_loss': [], 'mape': []}
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            mape = torch.mean(torch.abs((pred - target) / (target + 1e-6))) * 100
            ...
            metrics['mape'].append(mape.item())

        scheduler.step(r2.item())
        print(f"Epoch {epoch+1}/{num_epochs} | Train Loss: {total_loss / len(train_loader):.4f} | R²: {r2:.3f} | Abs MAE: {abs_mae:.4f} | Rel MAE: {rel_mae:.4f} | MAPE: {mape:.2f}% | 1-Corr: {corr_loss:.4f}")
        ...
        # Guardar modelo cada 30 épocas
        if (epoch + 1) % 30 == 0:
            torch.save(model.state_dict(), f'checkpoints/model_epoch_{epoch+1:03d}.pth')
